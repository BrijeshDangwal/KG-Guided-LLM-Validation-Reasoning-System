...
def step7_aggregate(
    ...
    if len(evidence_df) == 0:
        ...
        # max confidence per label (helps contradiction-first decisions)
        mx = evidence_df.pivot_table(
            index="id", columns="label", values="confidence", aggfunc="max", fill_value=0.0
        )
        for col in ["SUPPORT", "CONTRADICT", "NEUTRAL"]:
        ...
        mx = base.merge(mx, on="id", how="left").fillna(
            {"support_max": 0.0, "contradict_max": 0.0, "neutral_max": 0.0}
        )
        cnt[["support_n", "contradict_n", "neutral_n"]] = cnt[
            ["support_n", "contradict_n", "neutral_n"]
        ].astype(int)

    features = agg.merge(cnt, on="id", how="left").merge(mx, on="id", how="left")
    ...
    contra_max_thr = float(os.getenv("KDSH_STEP7_CONTRA_MAX_THR", "0.82"))
    contra_min_n = int(os.getenv("KDSH_STEP7_CONTRA_MIN_N", "1"))
    contra_margin = float(os.getenv("KDSH_STEP7_CONTRA_MARGIN", "0.06"))
    ...
    def decide_pred(r) -> int:
        ...
        if (int(r.get("support_n", 0)) + int(r.get("neutral_n", 0)) + int(r.get("contradict_n", 0))) == 0:
            return 0

        # contradiction-first gate: strong single contradiction beats weak support
        cmax = float(r.get("contradict_max", 0.0) or 0.0)
        smax = float(r.get("support_max", 0.0) or 0.0)
        cn = int(r.get("contradict_n", 0) or 0)
        if cn >= contra_min_n and cmax >= contra_max_thr and cmax >= smax + contra_margin:
            return 0
    ...
    def short_rationale(r) -> str:
        ...
        cn = int(r.get("contradict_n", 0) or 0)
        if cn >= contra_min_n and cmax >= contra_max_thr and cmax >= smax + contra_margin:
            return f"Strong contradiction detected (cmax={cmax:.2f} >= {contra_max_thr:.2f}); predicting 0."
